...
class FinalKGMapper:
    ...
    def __init__(self):
        ...
        self.bs_rules = {
            ...
            r'subchapter\s+s\s+income\s+tax\s+deposit[—-]current\s+portion': ('Other', 'current_assets'),
    ...
    def ask_ollama_for_section(self, description: str) -> str:
        ...
        try:
            ...
            template_sections = {
                'current_assets': ['Cash and equivalents', 'Accounts Receivable', 'Prepaid Expenses', 'Inventory', 'Investments'],
                'noncurrent_assets': ['Net PPE', 'Goodwill', 'Intangibles'], 
                'current_liabilities': ['Accounts Payable', 'Accrued Interest', 'Short term Borrowing', 'Current Portion of Long Term Debt'],
                'noncurrent_liabilities': ['Long Term Debt', 'Deferred income taxes'],
                'equity': ['Common Stock', 'Retained Earnings', 'Paid in Capital']
            }
            
            prompt = f"""You are a financial statement analyst. I need to classify this balance sheet line item into the correct section.

LINE ITEM: "{description}"

BALANCE SHEET SECTIONS AND EXAMPLES:
- current_assets: {template_sections['current_assets']}
- noncurrent_assets: {template_sections['noncurrent_assets']}  
- current_liabilities: {template_sections['current_liabilities']}
- noncurrent_liabilities: {template_sections['noncurrent_liabilities']}
- equity: {template_sections['equity']}

Please respond with ONLY the section name (e.g., "current_assets") based on standard accounting principles. Consider:
1. Current items are those due/realizable within one year
2. Assets are resources owned by the company
3. Liabilities are obligations owed by the company
4. Equity represents ownership interests
